...
async function acquireToken(credentials: ApiCredentials): Promise<TokenResponse> {
  try {
    ...
    params.append('grant_type', 'password'); // Use password grant type as specified in the API docs
    // Use the exact parameter names expected by the REI API CCA
    params.append('apikey', credentials.apiKey);
    params.append('integratorID', credentials.integratorId);
    params.append('secretkey', credentials.secretKey);
    
    // Make the OAuth token request with the correct format
...
export function tokenNeedsRefresh(credentials: ApiCredentials): boolean {
  ...
  return cachedEntry.refreshAt <= Date.now();
}
