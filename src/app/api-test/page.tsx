...
import { ReiApiCcaClient } from "@/lib/api/rei-api-cca";
import {
...
export default function ApiTestPage() {
  ...
  const [selectedOffice, setSelectedOffice] = useState<"AZURA" | "BLUE_OCEAN">(
    "AZURA"
  );
  ...
  const [lookups, setLookups] = useState<Record<string, unknown> | null>(null);
  ...
  const testAuthentication = async () => {
    ...
    try {
      ...
      console.log(
        `[Client] Authentication test result for ${selectedOffice}:`,
        result
      );
      ...
      console.error(
        `[Client] Authentication test failed for ${selectedOffice}:`,
        error
      );
      setAuthStatus("error");
  ...
  const testFetchProperties = async () => {
    ...
    try {
      ...
      console.log(
        `[Client] Properties fetch result for ${selectedOffice}:`,
        result
      );
      ...
      console.error(
        `[Client] Fetch properties failed for ${selectedOffice}:`,
        error
      );
  ...
  const testFetchAgents = async () => {
    ...
    try {
      ...
      console.log(
        `[Client] Agents fetch result for ${selectedOffice}:`,
        result
      );
      setAgents(result);
      ...
      console.error(
        `[Client] Fetch agents failed for ${selectedOffice}:`,
        error
      );
  ...
  const testFetchLookups = async () => {
    ...
    try {
      ...
      console.log(
        `[Client] Lookups fetch result for ${selectedOffice}:`,
        result
      );
      setLookups(result);
      ...
      console.error(
        `[Client] Fetch lookups failed for ${selectedOffice}:`,
        error
      );
