  ...
  try {
    ...
    if (!integratorId) {
      return NextResponse.json(
        { status: 'error', message: 'Missing required parameter: integratorId' },
        { status: 400 }
      );
    }
    ...
    try {
      ...
      const apiRootResponse = await axios.get(credentials.apiUrl.replace('/api/v1', ''), {
        headers: {
      ...
      console.log('[Server] API root response:', {
        ...
        data: typeof apiRootResponse.data === 'string' ? apiRootResponse.data.substring(0, 100) + '...' : apiRootResponse.data
      });
    } catch (error) {
      ...
      if (axios.isAxiosError(apiRootError) && apiRootError.response) {
        console.error('[Server] API root response status:', apiRootError.response.status);
        console.error('[Server] API root response data:', apiRootError.response.data);
      }
    }
