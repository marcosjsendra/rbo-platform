...
import { REMAX_AZURA_CREDENTIALS, REMAX_BLUE_OCEAN_CREDENTIALS } from '@/lib/api';
...
export async function GET(request: NextRequest) {
  ...
  if (testAuth) {
    try {
      // Get credentials and test authentication by getting a token
      const credentials = getCredentialsForIntegrator(testAuth);
      const token = await ensureValidToken(testAuth);
      ...
      console.error(`[Server] Authentication test failed for ${testAuth}:`, error);
      return NextResponse.json(
        { 
          status: 'error', 
  ...
  if (!endpoint || !integratorId) {
    return NextResponse.json(
      { status: 'error', message: 'Missing required parameters: endpoint and integratorId' },
      { status: 400 }
    );
  }
  ...
  try {
    ...
    console.log(`[Server] Forwarding request to REI API CCA: ${formattedEndpoint}`);
    const response = await axios.get(`${credentials.apiUrl}/${formattedEndpoint}`, {
      headers: {
